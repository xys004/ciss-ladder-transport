#!/usr/bin/env python3
"""Parallel search for chirality-odd physical transport polarization in the Hermitian 4x4 model."""
...
class Base:
    ...
    @property
    def eps1(self) -> float: return self.eps_avg - 0.5 * self.delta_eps
    @property
    def eps2(self) -> float: return self.eps_avg + 0.5 * self.delta_eps
...
def mus(bias: float, convention: str = "symmetric") -> tuple[float, float]:
    if convention == "symmetric": return 0.5 * bias, -0.5 * bias
    return 0.0, -bias
...
def d_values(b: Base, chi: int) -> tuple[complex, complex]:
    ...
    if b.map_type in ["A", "B", "C"]:
        ...
        if chi == 1: return Du, Dd
        if b.map_type == "A": return L * aup * np.exp(-1j * tu), -L * adown * np.exp(1j * td)
        if b.map_type == "B": return -L * aup * np.exp(-1j * tu), L * adown * np.exp(1j * td)
...
def currents_grid(b: Base, chi: int, n: int = 67, convention: str = "symmetric") -> dict:
    ...
    local = []
    for p in pts: local += [p - 5 * width, p, p + 5 * width]
...
def rand_log(rng, lo, hi):
    return 10 ** rng.uniform(math.log10(lo), math.log10(hi))

def random_base(rng, map_type: str) -> Base:
    V = rand_log(rng, 0.001, 0.2)
    L = rand_log(rng, 0.0005, 0.05)
    GL = rand_log(rng, 1e-5, 1e-2)
    GR = rand_log(rng, 1e-5, 1e-2)
    ea = rng.uniform(-0.1, 0.1)
    de = rng.uniform(-0.1, 0.1)
    bias = rng.choice([-1, 1]) * rand_log(rng, 0.02, 0.5)
    if map_type in ["A", "B", "C"]:
        params = (rand_log(rng, 0.05, 10), rng.uniform(-math.pi, math.pi), rng.uniform(-math.pi, math.pi))
    elif map_type == "D":
        params = (rand_log(rng, 0.05, 10), rng.uniform(-math.pi, math.pi), rng.uniform(-math.pi, math.pi), rng.uniform(-math.pi, math.pi))
    else:
        amps = [0.0 if rng.random() < 0.1 else rand_log(rng, 0.02, 10) for _ in range(4)]
...
def worker_task(map_type: str, seed: int, n: int) -> list[dict]:
    rng = np.random.default_rng(seed)
    results = []
    for _ in range(n):
        b = random_base(rng, map_type)
        res = eval_fast(b)
        results.append(res)
    return results
...
def tags_for(b: Base, Du: complex, Dd: complex) -> str:
    ...
    if gavg < 1e-4: tags.append("weak_contact")
    if b.V / gavg > 100: tags.append("molecule_contact_decoupled")
    ratio = max(abs(Du), abs(Dd)) / max(min(abs(Du), abs(Dd)), 1e-15)
    if ratio > 100: tags.append("extreme_D_asymmetry")
    return ";".join(tags)
...
def validate(rank: int, b: Base) -> dict:
    ...
    fail = []
    if abs(cp["Itot"]) <= 1e-8 or abs(cm["Itot"]) <= 1e-8: fail.append("finite_current")
    if abs(ctrl_p["zero_Iup"]) > 1e-10 or abs(ctrl_p["zero_Idown"]) > 1e-10 or abs(ctrl_m["zero_Iup"]) > 1e-10 or abs(ctrl_m["zero_Idown"]) > 1e-10: fail.append("zero_bias")
    if abs(ctrl_p["P_TRS"]) > 1e-8 or abs(ctrl_m["P_TRS"]) > 1e-8: fail.append("TRS")
    if abs(ctrl_p["P_noSO"]) > 1e-8 or abs(ctrl_m["P_noSO"]) > 1e-8: fail.append("noSO")
    if not (posp and posm): fail.append("positivity")
    if abs(cpw["P"] - cp["P"]) > max(1e-5, 0.02 * abs(cp["P"])) or abs(cmw["P"] - cm["P"]) > max(1e-5, 0.02 * abs(cm["P"])): fail.append("integration_stability")
    tags = tags_for(b, Du_p, Dd_p)
    return {
        "rank": rank, "map_type": b.map_type, "score": score, "score_loose": loose, "P_plus": cp["P"], "P_minus": cm["P"],
        "absP_min": min(abs(cp["P"]), abs(cm["P"])), "chirality_score": chir,
        "Iup_plus": cp["Iup"], "Idown_plus": cp["Idown"], "Itot_plus": cp["Itot"],
        "Iup_minus": cm["Iup"], "Idown_minus": cm["Idown"], "Itot_minus": cm["Itot"],
        ...
        "A_peak_plus": Apeak_p, "A_peak_minus": Apeak_m,
        "V": b.V, "Lambda": b.Lambda, "Gamma_L": b.GammaL, "Gamma_R": b.GammaR,
        "eps1": b.eps1, "eps2": b.eps2, "eps_avg": b.eps_avg, "delta_eps": b.delta_eps, "bias": b.bias,
        "Du_plus_real": Du_p.real, "Du_plus_imag": Du_p.imag, "Dd_plus_real": Dd_p.real, "Dd_plus_imag": Dd_p.imag,
        "Du_minus_real": Du_m.real, "Du_minus_imag": Du_m.imag, "Dd_minus_real": Dd_m.real, "Dd_minus_imag": Dd_m.imag,
        ...
        "P_TRS_control_plus": ctrl_p["P_TRS"], "P_TRS_control_minus": ctrl_m["P_TRS"],
        "P_noSO_plus": ctrl_p["P_noSO"], "P_noSO_minus": ctrl_m["P_noSO"],
        "zero_bias_Iup_plus": ctrl_p["zero_Iup"], "zero_bias_Idown_plus": ctrl_p["zero_Idown"],
        "zero_bias_Iup_minus": ctrl_m["zero_Iup"], "zero_bias_Idown_minus": ctrl_m["zero_Idown"],
        "passes_filters": len(fail) == 0, "tags": tags, "failure_reason": ";".join(fail), "params": repr(b.params),
    }
...
def main():
    ...
    parser.add_argument("--n-simple", type=int, default=10000)
    parser.add_argument("--n-map-e", type=int, default=10000)
    parser.add_argument("--workers", type=int, default=os.cpu_count())
    parser.add_argument("--validate-top", type=int, default=100)
    parser.add_argument("--outdir", type=str, default="chiral_physical_results")
    parser.add_argument("--seed", type=int, default=20260430)
    args = parser.parse_args()

    out = Path(args.outdir)
    out.mkdir(parents=True, exist_ok=True)
    partials_dir = out / "partials"
    ...
    # Simple maps
    simple_maps = ["A", "B", "C", "D"]
    total_simple = args.n_simple
    per_map = total_simple // len(simple_maps)
    
    for mt in simple_maps:
        rem = per_map
        while rem > 0:
    ...
    rem_e = args.n_map_e
    while rem_e > 0:
        c = min(rem_e, chunk_size)
        tasks.append(("E", c))
        rem_e -= c
        
    seeds = seed_seq.spawn(len(tasks))
    print(f"Submitting {len(tasks)} chunks to {args.workers} workers...")
    
    fast = []
    with ProcessPoolExecutor(max_workers=args.workers) as exc:
        futures = [exc.submit(worker_task, mt, s.generate_state(1)[0], n) for (mt, n), s in zip(tasks, seeds)]
        for i, f in enumerate(futures):
            fast.extend(f.result())
            if (i+1) % max(1, len(tasks)//20) == 0:
                print(f"Completed {i+1}/{len(tasks)} chunks")
    ...
    print("Validating top candidates...")
    rows = []
    seen = set()
    for r in fast:
        if len(rows) >= args.validate_top:
            break
        b = r["base"]
    ...
    report = [
        ...
        f"Fast random search: maps A-D {args.n_simple} total plus map E {args.n_map_e} = {args.n_simple+args.n_map_e}. Strict validated: {len(rows)}.",
    ...
    report += [
        ...
        "K. Recommendation: use 'moderate chirality-odd transport polarization' only if the good-candidate magnitude is acceptable."
    ]
