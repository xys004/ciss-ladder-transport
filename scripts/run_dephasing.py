"""Run the cleaned dephasing calculation for the legacy Gz workflow."""
...
from ciss_ladder_transport.randomness import build_dephasing_samples


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--sites", type=int, default=91, help="Number of sites per chain.")
    parser.add_argument("--realizations", type=int, default=10000, help="Number of dephasing realizations.")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPO_ROOT / "outputs" / "dephasing",
        help="Directory where the cleaned dephasing outputs will be written.",
    )
...
def main() -> None:
    ...
    samples = build_dephasing_samples(
    ...
    dat_path = output_dir / f"trans_SO_decoheren_Z_N={args.sites}.dat"
    csv_path = output_dir / f"data_decoheren_N{args.sites}.csv"
    runtime_path = output_dir / f"runtime_dephasing_N{args.sites}.txt"
