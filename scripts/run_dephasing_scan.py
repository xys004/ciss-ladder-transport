"""Compute a phenomenological dephasing-averaged spectral T^z(E) dataset.

This script corresponds to the dephasing workflow used to generate spin-z
transport kernels before the later Landauer-type integration step.

Physical regime: dephasing averaged.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eta-d", type=float, default=1.0)
...
def main() -> int:
    ...
    parameters = make_default_dephasing_parameters(num_sites=args.sites)
    ...
    samples = build_dephasing_realizations(args.sites, args.realizations, eta_d=args.eta_d, seed=args.seed)
    ...
    csv_path = save_spectral_kernel_csv(output_dir / f"fig4_Tz_dephasing_N{args.sites}_eta{args.eta_d}.csv", tz_frame)
    metadata = {
        "script": "scripts/run_dephasing_scan.py",
        "regime": "dephasing_averaged",
        "sites": args.sites,
    ...
    save_metadata_json(output_dir / f"fig4_Tz_dephasing_N{args.sites}_eta{args.eta_d}.json", metadata)
